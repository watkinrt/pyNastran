...
def components(card, ifield, fieldname):
    ...
    assert isinstance(svalue, string_types), 'card=%s type(%s=%s)=%s' % (str(card), fieldname, ifield, svalue)
...
def integer(card, ifield, fieldname):
    ...
    #try:
    svalue = card.field(ifield)
    #assert isinstance(svalue, string_types), 'card=%s type(%s=%s)=%s' % (str(card), fieldname, ifield, type(svalue))
    #print('itenger = %r' % svalue)
    #except IndexError:
...
def double(card, ifield, fieldname):
    ...
    #except IndexError:
        #raise SyntaxError('%s (field #%s) on card must be a float, but doesnt exist..\n'
                          #'card=%s' % (fieldname, ifield, card))
    return double_svalue(svalue, card, ifield, fieldname)

def double_svalue(svalue, card, ifield, fieldname):
    if svalue.isdigit():
        # if only int
...
def integer_or_double(card, ifield, fieldname):
    ...
    #except IndexError:
        #raise SyntaxError('%s (field #%s) on card must be an integer or float.\n'
                          #'card=%s' % (fieldname, ifield, card) )
    return integer_or_double_svalue(svalue, card, ifield, fieldname)

def integer_or_double_svalue(svalue, card, ifield, fieldname):
    if '.' in svalue:
        # float/exponent
        try:
            value = double_svalue(svalue, card, ifield, fieldname)
        except ValueError:
            dtype = _get_dtype(svalue)
...
def integer_double_or_blank(card, ifield, fieldname, default=None):
    ...
    #assert isinstance(fieldname, string_types), type(fieldname)
    #try:
    svalue = card.field(ifield).strip()
    #except IndexError:
    #    return default
...
def integer_or_string(card, ifield, fieldname):
    ...
    #try:
    svalue = card.field(ifield).strip()
    #print('integer_or_string : svalue=%r' % svalue)
    assert isinstance(svalue, string_types), 'card=%s type(%s=%s)=%s' % (str(card), fieldname, ifield, svalue)
    #except IndexError:
        #msg = ('%s (field #%s) on card must be an integer or string.\n'
               #'card=%s' % (fieldname, ifield, card))
        #raise SyntaxError(msg)
    return integer_or_string_svalue(svalue, card, ifield, fieldname)

def integer_or_string_svalue(svalue, card, ifield, fieldname):
    try:
        # int
        value = int(svalue)
    except ValueError:
...
def string(card, ifield, fieldname):
    ...
    if svalue.isdigit() or '.' in svalue:
        value = integer_or_double_svalue(svalue, card, ifield, fieldname)
